...
class InkHandler {
    ...
    async _handleLink(user, linkId){
        ...
        GameBroker.send('action', {
            userId: user.id,
            action: JSON.stringify(actionLib.link(link.url))
        });
    }
    ...
    async _handleMeeting(user, meetingId){
        ...
        GameBroker.send('action', {
            userId: user.id,
            action: JSON.stringify(actionLib.meeting(meeting))
        });
    }
    ...
    async _handleCloseMeeting(user){
        ...
        GameBroker.send('action', {
            userId: user.id,
            action: JSON.stringify(actionLib.closeMeeting())
        });
    }
    ...
    async _handleImage(user,imageId){
        ...
        GameBroker.send('action', {
            userId: user.id,
            action: JSON.stringify(actionLib.image(image.url, image.display_name, image.description))
        });
    }
    async _handleText(user, documentId, location, duration){
        ...
        GameBroker.send('action', {
            userId: user.id,
            action: JSON.stringify(await actionLib.text({
