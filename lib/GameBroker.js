...
class GameBroker extends EventEmitter{
    ...
    async send(gameId, type, data){
        const self = this;
        if (self.type === 'redis'){
            const publishAsync = promisify(self.publisher.publish).bind(self.publisher);
            await publishAsync(self.channel, JSON.stringify({gameId: gameId, type: type, payload: data}));
        } else {
            self.emit('message', data);
        }
    }
    ...
    async clean(){
        const self = this;
        const connections = await models.connection.find();
