...
exports.validate = async function validate(gameId){

    const screens = _.indexBy((await Promise.all(
        (await models.screen.find({game_id: gameId}))
            .filter(screen => {return !screen.template;})
            .map( async screen => {
                return gameEngine.getScreenRecord(screen.id);
            })
    )), 'id');
    const codes = await models.code.find({game_id: gameId});
    const links = await models.link.find({game_id: gameId});
    const meetings = await models.meeting.find({game_id: gameId});
    const groups = await models.group.find({game_id: gameId});
    const triggers = await models.trigger.find({game_id: gameId});
    const inks = await models.ink.find({game_id: gameId});
    ...
    const startScreen = await models.screen.getStart(gameId);
