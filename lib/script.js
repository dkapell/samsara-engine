...
async function getSandbox(player, run){
    ...
    return {
        ...
        player: (await models.user.get(run.game_id, player.user_id)).name,
        character: player.character,
        groups: player.groups,
        screen_id: player.screen_id,
        screens: (await models.screen.find({game_id: run.game_id})).filter(screen => {return !screen.template;}),
...
exports.runAction = async function runAction(gameId, script, player, dryrun){
    ...
    if (sandbox.output.console){
        console.log(`Script Output: ${sandbox.output.console} ${script}`);
    }
