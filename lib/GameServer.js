...
class GameServer {
    ...
    async handleMessage(ws, user, data){
        ...
        switch(data.action){
            ...
            case 'ink': await self.handleInk(ws, user, data); break;
        }
    ...
    async runTrigger(trigger, user){
        ...
        for(const action of actions){
            ...
            if (action.action === 'runupdate'){
                ...
                await self.runInkStory(action.ink_id, user, action.options);
    ...
    async runInkStory(inkId, user, options, ws){
        const self = this;
        const doc = JSON.stringify({
            action: 'ink',
            type: 'open',
