            ...
            switch(data.type){
                case 'message': {
                    ...
                    if (data.payload.type === 'report'){
                        const staff = await models.user.listGms(data.gameId);
                        ...
                        const recipients = await chat.getRecipients(data.payload, data.gameId);
                        await async.each(recipients, async userId => {
                            const recipient = await models.user.get(data.gameId, userId);
                ...
                case 'playerupdate': {
                    await self.sender.playerUpdate(data.gameId);
                    break;
                }
            }
        } catch (err) {
