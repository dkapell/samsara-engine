'use strict';
const _ = require('underscore');
const async = require('async');
const util = require('util');
const uuid = require('uuid');
const config = require('config');

const models = require('../models');
const gameEngine = require('../gameEngine');
const GameBroker = require('../GameBroker');
const chat = require('../chat');
const script = require('../script');

class MessageSender{
    constructor(clients){
        this.clients = clients;
        this.messageTimers = {
            locations: {},
            meetings: {}
        };
    }

    // Send chat messages to all connected clients
    chat(userId, message, ws){
        const self = this;
        const messageCopy = _.clone(message);

        if (userId === message.user_id){
            messageCopy.self = true;
        }

        const doc = JSON.stringify({
            action: 'chat',
            messages: [messageCopy],
            userId: userId
        });

        if (ws){
            ws.send(doc);
        } else {
            self.send(doc, userId);
        }
    }



    async chatLocations(userId, ws){
        const self = this;
        if (self.messageTimers.locations && self.messageTimers.locations[userId]){
            return;
        }
        self.messageTimers.locations[userId] = setTimeout(async () => {
            const doc = JSON.stringify({
                action: 'chat',
                locations: await chat.getLocations(userId),
                userId: userId
            });
            if (ws){
                ws.send(doc);
            } else {
                self.send(doc, userId);
            }
            delete self.messageTimers.locations[userId];
        }, 50);
    }

    async gameData(runId, userId){
        const self = this;
        // Send updated screen counts
        let players = [];
        if (userId){
            const player = await models.player.get(userId);
            if(player){
                players.push(player);
            }
        } else {
            players = await models.player.find({run_id:runId});
        }

        let delay = 0;
        if (players.length > 1){
            delay = 50;
        }

        if (self.messageTimers.gamedata){
            return;
        }

        self.messageTimers.gamedata = setTimeout(async () => {
            const screenCounts = await gameEngine.getScreenCounts(runId);
            await async.each(players, async player =>{
                const screen = await gameEngine.getScreen(player.user_id);
                const doc = JSON.stringify({
                    action: 'gamedata',
                    gamedata:{
                        screen: screenCounts,
                        player: _.has(screen.player.data, 'public')?screen.player.data.public:{},
                        run: _.has(screen.run.data, 'public')?screen.run.data.public:{},
                        character: screen.player.character,
                        user: {
                            name: (await models.user.get(player.user_id)).name
                        },
                        groups: _.pluck(screen.player.groups.filter(group=>{return group.chat;}), 'name')

                    }
                });
                self.send(doc, player.user_id);
            });
            delete self.messageTimers.gamedata;
        }, delay);
    }

    // Send a message to all connected clients for a specified user
    send(message, userId){
        const self = this;
        if (userId){
            userId = userId.toString();
            for (const clientId in self.clients[userId]){
                self.clients[userId][clientId].send(message);
            }
        } else {
            for (const userId in self.clients){
                for (const clientId in self.clients[userId]){
                    self.clients[userId][clientId].send(message);
                }
            }
        }
    }

    removeMessage(message_id, options){
        const self = this;
        if (!options){
            options = {};
        }
        const doc = JSON.stringify({
            action: 'chat',
            remove: message_id
        });
        if (_.has(options, 'ws')){
            options.ws.send(doc);

        } else {
            self.send(doc, options.userId);
        }
    }

    toast(message, options, ws){
        const self = this;

        if (!options){
            options = {};
        }
        const doc = {
            id: uuid.v4(),
            action: 'toast',
            message: message,
            duration: options.duration?options.duration:null,
            from: options.from?options.from:config.get('app.name')
        };

        self.action(options.userId, doc, ws);
    }

    async screen(userId, ws, force){
        const self = this;

        const screen = (await gameEngine.getScreen(userId));
        const current = screen.current;
        if (!current){
            const doc = self.defaultJSON();
            doc.chat = screen.chat;
            doc.chatSidebar = screen.chatSidebar;
            doc.chatExpanded = screen.chatExpanded;
            return self.action(userId, doc, ws);
        }

        const doc = {
            player: true,
            action: 'show page',
            screen:{
                id: current.id,
                description: current.description,
                showCode: current.codes && current.codes.length > 0,
                chatSidebar: !(current.start||current.finish),
                chatExpanded:true,
                chat: current.chat
            },
            gamedata: {
                player: _.has(screen.player.data, 'public')?screen.player.data.public:{},
                run: _.has(screen.run.data, 'public')?screen.run.data.public:{},
                character: screen.player.character,
                user: {
                    name: (await models.user.get(userId)).name
                }
            }
        };
        if (current.show_name){
            doc.screen.name = current.name;
        }
        if (force){
            doc.force = true;
        }

        if (current.image_id){
            doc.screen.image_id = current.image_id;
            doc.screen.image = { url: current.image.url };

            doc.screen.map = [];
            for (const area of current.map){
                if (area.condition){
                    if (!await script.runCheck(area.condition, screen.player)){
                        continue;
                    }
                }
                if (area.group_id && !_.findWhere(screen.player.groups, {id: area.group_id})){
                    continue;
                }
                const areaDoc = {
                    id: area.uuid,
                    shape: area.shape,
                    coords: area.coords,
                    name: area.name
                };

                const areaMeeting = await gameEngine.getAreaMeeting(area, userId);
                if (areaMeeting){
                    areaDoc.meeting = areaMeeting;
                }
                //todo - check for meeting
                doc.screen.map.push(areaDoc);
            }
        }
        return self.action(userId, doc, ws);
    }

    defaultJSON(){
        return {
            player: false,
            action: 'show default',
            chatSidebar:false,
        };
    }

    // Send an action to the broker or a specific socket
    async action(userId, actionDoc, ws){

        if (ws){
            ws.send(JSON.stringify(actionDoc));
        } else {
            return GameBroker.send('action', {
                userId: userId,
                action: JSON.stringify(actionDoc)
            });
        }
    }

    async message(message){
        return GameBroker.send('message', message);
    }

}

module.exports = MessageSender;
