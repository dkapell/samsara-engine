...
exports.getHistory = async function getHistory(user, options){
    ...
    if (user.player){
        ...
        if (!options.type || options.type === 'screen'){
            ...
            for (const message of await models.message.find({location:'screen', location_id: user.player.screen_id, run_id: user.player.run_id})){
        ...
        if (!options.type || options.type === 'screen'){
            ...
            for (const message of await models.message.find({location:'screen', run_id: run.id},  {limit:getLimit('screen', 100, options)})){
                messages.push(message);
            }
        }
        if (!options.type || options.type === 'group'){
    ...
    const cache = {
        ...
        screen: await models.screen.list(),
...
exports.getLocations = async function getLocations(userId){
    ...
    if (user.type === 'none'){
        ...
        const screens = await models.screen.listForChat();
