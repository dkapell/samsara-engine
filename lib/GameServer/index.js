...
class GameServer {
    ...
    async handleConnection(ws, request){
        ...
        await self.sender.screen(userId, ws);
    ...
    async handleMessage(ws, user, data){
        const self = this;
        const oldScreen = await gameEngine.getScreen(user.id);
        let force = false;
        switch(data.action){
        ...
        if (user.player){
            const newScreen = await gameEngine.getScreen(user.id);

            if (oldScreen.current.id !== newScreen.current.id || force){
                await self.sender.screen(user.id, null, force);
            ...
            if( newScreen.transitioning ){
                ...
                self.playerTimers[user.id] = setTimeout(async () => {
                    await self.sendScreen(user.id, null, force);
                    await self.sendLocationUpdate(user.player.run_id, oldScreen.current.id, newScreen.current.id);
    ...
    async sendScreen(userId, ws, force){
        const self = this;
        return self.sender.screen(userId, ws, force);
    }
    ...
    async sendLocationUpdate(runId, oldScreenId, newScreenId){
        const self = this;
        await GameBroker.send('screenchange', {
            runId: runId,
