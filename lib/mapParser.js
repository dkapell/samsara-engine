...
exports.parseMap = async function(input, gameId){
    ...
    for (const id in input){
        ...
        if (_.has(area, 'condition')){
            const verified = await scriptRunner.verify(area.condition);
...
exports.parseActions = async function(input, gameId){
    ...
    for (const actionId in input){
        ...
        if (_.has(row, 'condition_id')){
            ...
            if (action.condition_id === -2 && _.has(row, 'condition')){
                const verified = await scriptRunner.verify(row.condition);
                if (!verified.verified){
                    throw new Error('Script Error: ' + verified.errors);
                }
        ...
        switch (row.type){
            ...
            case 'script':{
                ...
                if (action.function_id === -2 && _.has(row.script)){
                    const verified = await scriptRunner.verify(row.script);
                    if (!verified.verified){
