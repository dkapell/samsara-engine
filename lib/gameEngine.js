...
exports.runTrigger = async function runTrigger(triggerId, userId, gameId){
    ...
    const trigger = await models.trigger.find({game_id: gameId, id: triggerId});
...
exports.updateTriggers = async function updateTriggers(userId, gameId) {
    ...
    if (!triggers) {
        triggers = (await models.trigger.find({game_id: gameId})).filter( trigger => { return trigger.player; } );
    ...
    for (const action of actions){
        ...
        switch(action.type){
            ...
            case 'script':{
                let result = null;
                try{
                    result = await script.runAction(gameId, action.script, screen.player);
...
exports.getAreaMeeting = async function getAreaMeeting(area, userId, gameId){
    ...
    await async.each(actions, async (action) => {
        switch (action.type){
            ...
            case 'script':{
                const player = await models.player.getByUser(userId, gameId);
                const result = await script.runAction(player.game_id, action.script, player);
