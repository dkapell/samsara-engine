...
class InkEngine extends EventEmitter{
    ...
    async init() {
        const self = this;
        const ink = await models.ink.findOne({game_id: self.gameId, id: self.inkId});
        self.story = new inkjs.Story(ink.compiled);
        const variables = await models.variable.listInk(self.gameId);
        const functions = await models.function.find({game_id:self.gameId, type:'ink'});
        const player = await models.player.getByUser(self.userId, self.gameId);
        self.run = await models.run.get(player.run_id);
        ...
        for (const func of functions){
            try{
                self.story.BindExternalFunction(func.name, function(){
                    const args = Array.from(arguments);
                    return scriptRunner.runInk(self.gameId, func.content, self.player, self.run, args);
                }, false);
            } catch (e){
    ...
    async continue(){
        ...
        self.player = await models.player.getByUser(self.userId, self.gameId);
        self.run = await models.run.get(self.runId);
