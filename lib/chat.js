...
exports.getRecipients = async function getRecipients(message, gameId){
    let  recipients = [message.user_id];
    if (message.location !== 'direct'){
        const users = await models.user.listGms(gameId);
...
exports.getHistory = async function getHistory(user, gameId, options){
    ...
    const cache = {
        user: await models.user.find(gameId),
    ...
    if (user.type === 'none'){
        ...
        doc.group = await async.mapLimit(groups, 10, async group => {
            return {
                id: group.id,
                name: group.name,
                users: await getPlayerList(await models.player.listByGroupAndRun(group.id, run.id), 'player', gameId)
            };
        });
        ...
        const screen = await models.screen.get(location_id);
        if (screen.chat){
            doc.current = await getPlayerList(await models.player.find({run_id: run.id, screen_id: screen.id}), 'player', gameId);
        } else {
            doc.current = [];
        }
    } else {
        ...
        doc.group = await async.mapLimit(groups, 10, async group => {
            return {
                id: group.id,
                name: group.name,
                users: await getPlayerList(await models.player.listByGroupAndRun(group.id, run.id), 'full', gameId)
            };
        });
        ...
        doc.screen = await async.mapLimit(screens, 5, async screen => {
            return {
                id: screen.id,
                name: screen.name,
                users: await getPlayerList(await models.player.find({run_id: run.id, screen_id: screen.id}), 'full', gameId)
            };
        });
    }
    return doc;
...
async function getPlayerList(players, type, gameId){
    return await async.mapLimit(players, 5, async player => {
        const user = await models.user.get(gameId, player.user_id);
        return {
            id: player.user_id,
