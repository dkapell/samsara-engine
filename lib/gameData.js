...
exports.addNewVariable = async function addNewVariable(variable, oldVariable, gameId){
    if (variable.player){
    ...
    async function addVariable(type){
        const objects = await models[type].find({game_id:gameId});
        ...
        return async.each(objects, async function(item){
            if (!_.has(item, 'data') || !item.data || !_.has(item.data, 'public') || !_.has(item.data, 'private')) {
                item.data = await exports.getStartData(type, gameId);
            }
            if (oldVariable){
...
exports.getStartData = async function getStartData(type, gameId){

    const variables = await models.variable.find({player:(type==='player'), game_id: gameId});
...
exports.validate = async function validate(data, type, gameId){
    const variables = await models.variable.find({player:type==='player', game_id:gameId});
